...
uint32_t esp_app_main_tile_num;
uint32_t esp_app_setup_tile_num;
...
static void enter_esp_app_event_cb( lv_obj_t * obj, lv_event_t event );
static void enter_esp_widget_event_cb( lv_obj_t * obj, lv_event_t event );
...
void esp_app_setup( void ) {
    ...
    esp_app_main_tile_num = mainbar_add_app_tile( 1, 2, "esp3d app" );
    esp_app_setup_tile_num = esp_app_main_tile_num + 1;
    ...
    esp_app = app_register( "esp3d", &esp3d_64px, enter_esp_app_event_cb );
    app_set_indicator( esp_app, ICON_INDICATOR_OK );
    ...
    // use https://lvgl.io/tools/imageconverter to convert your images and set "true color with alpha" to get fancy images
    // the resulting c-file can put in /app/examples/images/ and declare it like LV_IMG_DECLARE( your_icon );
    example_widget = widget_register( "myapp", &example_app_64px, enter_esp_widget_event_cb );
...
static void enter_esp_app_event_cb( lv_obj_t * obj, lv_event_t event ) {
    switch( event ) {
        case( LV_EVENT_CLICKED ):       statusbar_hide( true );
                                        app_hide_indicator( esp_app );
                                        mainbar_jump_to_tilenumber( esp_app_main_tile_num, LV_ANIM_OFF );
                                        break;
    }    
}
